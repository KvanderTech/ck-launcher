    ...
    <div className="cosmetics-layout">
      ...
      <div className="cosmetics-content">
        <section className="cosmetics-panel">
          ...
          <div className="skin-library-toolbar"><button aria-pressed={libraryTab === "favorites"} className={libraryTab === "favorites" ? "skin-favorites-toggle active" : "skin-favorites-toggle"} onClick={() => setLibraryTab((current) => current === "favorites" ? "all" : "favorites")} type="button">★ Избранное <span>{skins.filter((skin) => skin.isFavorite).length}</span></button><label className="skin-search"><span>⌕</span><input aria-label="Поиск скинов" onChange={(event) => setQuery(event.target.value)} placeholder="Поиск по названию" value={query} /></label></div>
          <div className="saved-skin-grid cosmetics-skin-grid">
            ...
            {libraryTab === "all" && !query ? <button aria-label="Добавить скин" className="skin-library-empty" onClick={() => void addSkin()} title="Добавить скин" type="button"><span>＋</span></button> : null}
            {!visibleSkins.length && (libraryTab === "favorites" || Boolean(query.trim())) ? <div className="skin-library-no-results"><span>{libraryTab === "favorites" ? "☆" : "⌕"}</span><p>{libraryTab === "favorites" ? "Добавьте скины звёздочкой" : "Скины не найдены"}</p></div> : null}
          </div>
