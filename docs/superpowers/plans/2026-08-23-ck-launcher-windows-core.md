...
- OAuth: системный браузер, listener `127.0.0.1:0`, redirect `http://localhost:<port>/callback`, PKCE и проверка `state`.
...
Loopback привязывается только к `127.0.0.1:0`, но для authorize/token использует точный redirect `http://localhost:<выбранный порт>/callback` (зарегистрированный Entra URI: `http://localhost/callback`). Он принимает один callback, сравнивает state постоянным по времени сравнением, отвечает локальной HTML-страницей «Можно вернуться в ЦК Лаунчер», поддерживает явную отмену и закрывается по таймауту.
