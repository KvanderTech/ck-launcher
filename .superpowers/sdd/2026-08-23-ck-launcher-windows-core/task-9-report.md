...
## Исправления по ревью — round 4

- Сериализованная memory-очередь получила один общий awaitable Promise, который выполняется до подтверждения самого свежего desired-значения. Debounce-запуск безопасно поглощает ожидаемую ошибку после восстановления UI, а Play ожидает тот же Promise напрямую.
- Play немедленно снимает ожидающий debounce, запускает сохранение текущего desired memory и ждёт завершения всех in-flight/queued записей. Только после подтверждения последнего значения он заново читает актуальный профиль, выполняет полный `updateProfile` и запускает игру.
- Ошибка последнего memory-запроса завершает ожидание без deadlock, возвращает память к последнему backend-подтверждению и переводит главный экран в recoverable-состояние; `updateProfile` и `launchOrInstall` при этом не вызываются.
- RED/GREEN-регрессии покрывают обе границы: A уже выполняется, B стоит в очереди, Play ждёт подтверждения B и сохраняет/запускает профиль с B; отказ memory flush блокирует запуск и показывает доступный retry.
- Проверки: `npm test -- --reporter=verbose` — PASS, 7 files / 25 tests; `npm run build` — PASS, 51 module transformed. Rust/backend в round 4 не менялся.
