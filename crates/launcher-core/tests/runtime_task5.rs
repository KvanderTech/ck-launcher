...
impl ProcessRunner for FakeRunner {
    async fn run(
        ...
        args: &[&str],
        timeout: Duration,
    ) -> Result<ProcessOutput, ck_launcher_core::error::LauncherError> {
        ...
        self.versions
            ...
            .cloned()
            .ok_or_else(|| {
                ck_launcher_core::error::LauncherError::internal("unknown fake executable")
            })
    }
}
...
fn resolution_prefers_matching_managed_then_manual_then_system_and_never_mismatch() {
    ck_launcher_core::tasks::block_on(async {
