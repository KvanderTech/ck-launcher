...
pub async fn begin_microsoft_login(
    ...
    let (session, code) = crate::tasks::spawn_blocking(move || {
...
pub async fn cancel_microsoft_login(auth: &Arc<AuthService>) -> Result<(), LauncherError> {
    auth.cancel_login()
}
...
mod tests {
    ...
    fn switching_active_account_waits_for_the_shared_mutation_coordinator() {
        crate::tasks::block_on(async {
            ...
            let mut switching = crate::tasks::spawn({
