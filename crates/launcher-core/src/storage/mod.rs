...
impl Storage {
    ...
    pub async fn connect_file(database: &Path) -> Result<Self, LauncherError> {
        ...
        if database.is_file() {
            ...
            if current.unwrap_or(0) < latest {
                let backup = parent.join(format!(
                    "launcher-before-schema-{latest}-{:016x}.sqlite3",
                    rand::random::<u64>()
                ));
                sqlx::query("VACUUM INTO ?")
                    ...
                    .map_err(|_| {
                        LauncherError::new(
                            ...
                            "Не удалось создать резервную копию базы. Обновление остановлено.",
                            None,
                            true,
                        )
                    })?;
    ...
    pub async fn commit_pack(
        ...
        sqlx::query("INSERT INTO pack_sources (build_id,source_json) VALUES (?,?) ON CONFLICT(build_id) DO UPDATE SET source_json=excluded.source_json").bind(&build.id).bind(source).execute(&mut *tx).await.map_err(|_| LauncherError::storage_unavailable())?;
        if build.is_active {
