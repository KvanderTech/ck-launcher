...
pub fn sanitize(details: &str) -> String {
    ...
    let named_token = Regex::new(
        r#"(?i)\b(access_token|refresh_token|accessToken|refreshToken|client_secret|clientSecret|authorization|xbl3)(["']?\s*[:=]\s*)(?:"(?:\\.|[^"\\])*"|'(?:\\.|[^'\\])*'|[^\s,;&]+)"#,
    )
    .expect("named token redaction pattern is valid");

    let named = named_token.replace_all(&bearer, "$1$2[REDACTED]");
    let command_line = Regex::new(
        r#"(?i)(--(?:accessToken|refreshToken|clientSecret)\s+)(?:"[^"]*"|'[^']*'|[^\s]+)"#,
    )
    .expect("command-line token pattern")
    .replace_all(&named, "$1[REDACTED]");
    Regex::new(r#"(?i)XBL3\.0\s+x=[^;\s]+;[^\s,\"']+"#)
        .expect("Xbox token pattern")
        .replace_all(&command_line, "XBL3.0 [REDACTED]")
        .into_owned()
}
...
mod tests {
    ...
    #[test]
    fn redacts_json_cli_and_xbox_tokens() {
        let output = super::sanitize(
            r#"{"accessToken":"json-secret"} --accessToken cli-secret XBL3.0 x=user;xbox-secret"#,
        );
        for secret in ["json-secret", "cli-secret", "xbox-secret"] {
