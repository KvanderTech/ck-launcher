...
Первый стабильный выпуск: новый главный экран, постоянные библиотеки скинов по UUID, компактные плащи, исправленные описания и иконки Modrinth, этапы и проценты загрузок. [Изменения 1.0.0](docs/releases/v1.0.0.md).

## Предыдущие обновления

Компактная вкладка версий с установкой конкретного релиза из каждой строки, синхронный выбор и повтор установки после ошибки. Возвращены прежние контуры и подсветка иконок, скруглены значки сборок. Исправлены ошибки восстановления контента, отмены входа, живого журнала и установки обновлений. Подробнее: [изменения 0.2.4-beta.1](docs/releases/v0.2.4-beta.1.md) и [проверки стабильности](docs/audit/2026-09-08-stability.md).
...
Добавлена установка Forge через официальный клиентский установщик: сборки Modrinth и `.mrpack` с зависимостью `forge` больше не отклоняются как неподдерживаемые. Подготовка Forge выполняется отдельно от пользовательской `.minecraft`, с проверкой файлов и восстановлением через настройки сборки. [Устройство и ограничения](docs/FORGE.md).
...
- поддержка Vanilla, Fabric, Quilt и Forge при создании и запуске сборок (ограничения Forge — в [документации](docs/FORGE.md));
